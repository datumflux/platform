-- tables
local _G = _G
local string, debug, coroutine = string, debug, coroutine
	...
	while info do
		...
		elseif info.what == "Lua" then
			...
			if info.source and info.source:sub(1, 1) == "@" then
				dumper:add_f("(%d) Lua %s '%s' at file '%s:%d'\r\n", level_to_show, function_type, function_name, info.source:sub(2), info.currentline)
			elseif info.source and info.source:sub(1,1) == '#' then
				dumper:add_f("(%d) Lua %s '%s' at template '%s:%d'\r\n", level_to_show, function_type, function_name, info.source:sub(2), info.currentline)
			else
